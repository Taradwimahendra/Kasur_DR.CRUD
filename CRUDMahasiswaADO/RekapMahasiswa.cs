...
namespace CRUDMahasiswaADO
{
    public partial class RekapMahasiswa : Form
    {
        ...
        private void btnCetak_Click(object sender, EventArgs e)
        {
            try
            {
                Cursor.Current = Cursors.WaitCursor; // Indikator loading karena Crystal Report bisa lama
                string prodi = cmbProdi.SelectedValue != null ? cmbProdi.SelectedValue.ToString() : cmbProdi.Text;
                Report frm2 = new Report(prodi, dtpTanggalMasuk.Value);
                frm2.Show();
                this.Hide();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error saat membuka cetak data: " + ex.Message);
            }
            finally
            {
